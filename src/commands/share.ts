...
export function register(program: Command): void {
  program
    ...
    .description("Prepare an explicit Git share plan for selected active memories.")
    .option("--all-active", "Share all active memories in .brain.")
    .option("--include-source-evidence", "Include exact raw provenance blobs after local verification.")
    .action(async (memoryId: string | undefined, options: { allActive?: boolean; includeSourceEvidence?: boolean }) => {
      ...
      for (const warning of plan.warnings) output.write(`WARNING: ${warning}\n`);
